...
import { Topic, BasicIdeaFlags, CoreIdea } from '../../promptsData/Ideas';
...
const TopicLine: FC<TopicItem> = (props) => {
	...
	const ID = `PromptLine-Topic-${id}`;

	const maybeDelete = useCallback(() => {
		if(all.length <= 1) {
			return toaster({
				message: "Cannot delete: At least three Topics are required for the tool to function.",
				color: "danger",
				duration: 5000,
				position: "middle",
				toast
	...
	return (
		<IonItemSliding id={ID} style={style}>
			...
			<IonItemOptions side="end">
				...
				:
					<HaltButton errorMessage="At least three Topics are" />
				}
...
const TopicItems = memo(({index, style, data: ideas}: IdeaItem<Topic>) => {
	const idea = ideas[index];
	const { id, type } = idea;
	return <TopicLine key={`TopicLine:${type}/${id}`} item={idea} all={ideas} style={style} />;
}, areEqual);

const PromptsTopicsEdit: FC = () => {
	const [open, setOpen] = useState<boolean>(false);
	const toast = useIonToast();
	const topics = useAppSelector(state => state.writingPromptsSettings.ideas.topic);
	const dispatch = useAppDispatch();
	...
	return (
		<PromptsIdeasEdit ideas={topics} IdeaItems={TopicItems} title="Topics" setAddModalOpen={setOpen}>
			<PromptsAddModal
				modalOpen={open}
...
export default PromptsTopicsEdit;
